class Animal {
    constructor(glContext) {
      ...
      this.tailTipAngle = 0;
      this.headBob = 0;
      this.showNormals = false; // <-- added
    }
  
    render(globalMatrix) {
      const gl = this.gl;
  
      // Helper function to replace drawCube, keeps original colors
      /*const drawPart = (mat, color, normalTexture = 1) => {
        const part = new Cube();
        part.color = color; // original color
        part.textureNum = this.showNormals ? -3 : normalTexture;
        part.matrix = mat;
        part.normalMatrix.setInverseOf(part.matrix).transpose();
        gl.uniformMatrix4fv(u_NormalMatrix, false, part.normalMatrix.elements);
        part.renderfaster();
      };
      */
      const drawPart = (mat, color, textureNum = -2) => {
        const part = new Cube();
        part.color = color;
        part.textureNum = this.showNormals ? -3 : textureNum;
        part.matrix = mat;
        part.normalMatrix.setInverseOf(part.matrix).transpose();
        gl.uniformMatrix4fv(u_NormalMatrix, false, part.normalMatrix.elements);
        part.renderfaster();
      ...
      drawPart(tailBaseDrawMat, [0.85, 0.55, 0.55, 1.0]);
      ...
      drawPart(tailMidDrawMat, [0.85, 0.55, 0.55, 1.0]);
      ...
      drawPart(tailTipDrawMat, [0.85, 0.55, 0.55, 1.0]);
    }
  }
/*
class Animal {
